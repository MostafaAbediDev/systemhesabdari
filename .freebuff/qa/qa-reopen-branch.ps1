...
[MouseWin32]::RightClick([int]($r.X + $r.Width * 0.62), [int]($r.Y + $r.Height/2))
Wait-Ms 900
$editBtn = Find-ByAId $win 'EditButton'
if (-not $editBtn) { Write-Output "EDIT BUTTON NOT FOUND"; exit 1 }
Click-El $editBtn
Wait-Ms 2000

# dump values
$out = 'E:\NewVersiongithub\githubeVersionMerge\Code\systemhesabdari\.freebuff\qa\uia-branch-reopen.txt'
