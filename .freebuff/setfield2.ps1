param([string]$FieldId = 'FirstNameInput', [string]$Value = 'QA')
...
if (-not $outer) { Write-Output "OUTER_NOT_FOUND id=$FieldId"; exit 1 }
$inner = $null
foreach ($e in $all) {
...
if (-not $inner) { Write-Output "INNER_NOT_FOUND id=$FieldId"; exit 1 }
try {
