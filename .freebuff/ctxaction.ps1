param(
    [string]$Cell = 'ثصقصثق ثصقصثقصثق',
    [string]$Action = 'ویرایش',
    [string]$Out = 'E:\NewVersiongithub\githubeVersionMerge\Code\systemhesabdari\.freebuff\verify_ctx_action.txt'
)
...
# 1) find cell text and right-click
$all = Get-Tree
$found = $null
...
[void][Win32CtxA]::SetForegroundWindow($hwnd)
Start-Sleep -Milliseconds 400
[System.Windows.Forms.Cursor]::Position = New-Object System.Drawing.Point($x, $y)
Start-Sleep -Milliseconds 200
[Win32CtxA]::mouse_event(0x0008, 0, 0, 0, [UIntPtr]::Zero)
Start-Sleep -Milliseconds 80
[Win32CtxA]::mouse_event(0x0010, 0, 0, 0, [UIntPtr]::Zero)
[void]$sb.AppendLine("RCLICK '$Cell' at=$x,$y")
Start-Sleep -Milliseconds 2500

# 2) find menu item and click it
$all2 = Get-Tree
$menuItem = $null
foreach ($e in $all2) { if ($e.Current.Name -eq $Action -and $e.Current.BoundingRectangle.Width -gt 0) { $menuItem = $e; break } }
if (-not $menuItem) {
    [void]$sb.AppendLine("MENU_ITEM_NOT_FOUND '$Action'")
} else {
    ...
    Start-Sleep -Milliseconds 200
    [Win32CtxA]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
    Start-Sleep -Milliseconds 60
    [Win32CtxA]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    [void]$sb.AppendLine("CLICKED_MENU '$Action' at=$mx,$my")
}
Start-Sleep -Milliseconds 2500
$proc.Refresh()
[void]$sb.AppendLine("after: alive=$(-not $proc.HasExited) responding=$($proc.Responding) memMB=$([math]::Round($proc.WorkingSet64/1MB))")

# 3) count windows for this process (error dialogs would add windows)
